...
const TermTooltip = (props) => {

    const [open, setOpen] = useState(false);

    const term = terms.find(term => term.name === props.id)
            ...
            <Reference>
                {({ ref }) => (
                    <div className="relative inline-flex items-center cursor-pointer" ref={ref} onMouseEnter={() => setOpen(true)} onMouseLeave={() => setOpen(false)}>
                        ...
                        <div
                            className="-mt-2 ml-2 w-4 h-4 bg-white rounded-full flex items-center justify-center text-primary-500 bg-primary-100"
                            css={css`
                                &:hover {
